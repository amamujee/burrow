...
test.describe("Fruits", { tag: "@logic" }, () => {
  test("100 distinct fruits have local photos, provenance and complete profiles across six continents", () => {
    expect(pack.cards).toHaveLength(100);
    expect(new Set(pack.cards.map((card) => card.id)).size).toBe(100);
    expect(new Set(pack.cards.map((card) => card.name)).size).toBe(100);
  ...
  test("undocumented weight stays unknown, sorts last and never becomes a flavor-score weight", () => {
    ...
    expect(sorted.slice(0, 40).every((card) => weighted.has(card.id))).toBe(true);
    expect(sorted.slice(40).every((card) => !weighted.has(card.id))).toBe(true);
    expect(collectionOrderLabel(sorted)).toBe("Example weight · highest to lowest");
  });
  ...
  test("all difficulties generate fruit-only rounds with honest weights and matching comparisons", () => {
    ...
    for (let seed = 120; seed < 1000 && seen.size < 100; seed++) {
...
test("Fruits opens, plays, and displays sourced profiles on tablet and mobile", { tag: ["@browser", "@mobile"] }, async ({ page }, testInfo) => {
  ...
  await page.goto("/");
  await expect(page.getByText("100 fruits, from sweet to surprising", { exact: true })).toBeVisible();
  ...
  await page.evaluate(() => {
    ...
    active.progress.unlockedCards = ["fruits:apple", "fruits:lychee", "fruits:finger-lime", "fruits:rambutan", "fruits:mangosteen"];
  ...
  await expect(collection.getByRole("button", { name: "Show all rarities (100 cards)" })).toBeVisible();
  await collection.getByRole("button", { name: "Show all 100 cards", exact: true }).click();
