...
const sizeNote = (card) => `Approximate longest fruit-body dimension, not a species average or a measurement of this photo. ${card.size.rangeCm ? `Reference ${card.size.dimension} range: ${card.size.rangeCm.map(format).join("–")} cm. ` : ""}${card.size.note}`;
const references = (card) => card.sourceUrls.map((url) => {
  const isWeight = url === card.weight?.sourceUrl;
  const isSize = url === card.size.sourceUrl;
  const measurements = [isWeight && "weight", isSize && "size"].filter(Boolean);
  return {
    label: `${card.name} — ${measurements.length ? measurements.join(" and ") : "fruit"} reference`,
    url,
    note: [
...
const pack = {
  ...
  id: "fruits",
  title: "Fruits",
  summary: `Discover ${fruitCount} fruits, from familiar favorites to rare regional treasures, through size, weight, flavor, growing regions and surprising facts.`,
  ...
  sources: source.cards.flatMap(references),
  cards: source.cards.map((card) => {
    ...
    if (!Number.isFinite(card.size?.centimeters) || card.size.centimeters <= 0
      || !["length", "diameter"].includes(card.size.dimension) || !card.size.note
      || !card.sourceUrls.includes(card.size.sourceUrl)) {
      throw new Error(`${card.id}: size needs a positive measurement, dimension, reference and scope`);
    }
    if (card.size.rangeCm && (card.size.rangeCm.length !== 2
      || !card.size.rangeCm.every((value) => Number.isFinite(value) && value > 0)
      || card.size.rangeCm[0] > card.size.centimeters || card.size.rangeCm[1] < card.size.centimeters)) {
      throw new Error(`${card.id}: example size must lie inside its reference range`);
    }
    ...
    return {
      ...
      stats: [
        ...
        { id: "size-cm", label: "Example size", value: card.size.centimeters, unit: "cm", display: `~${format(card.size.centimeters)} cm`, direction: "higher", note: sizeNote(card) },
      ...
      details: [
        ...
        { label: "Size guide", value: source.methodology.size },
        { label: "Comparison guide", value: "Sizes and weights are examples, not fixed species averages, and may describe different specimens. Sweetness and tartness are game ratings, not sugar content or tastiness scores. Availability uses broad bands, not country counts." },
      ...
      metadata: {
        ...
        accuracyNote: `${sizeNote(card)} ${weightNote} ${ratingNote} Availability is an editorial band, not a country count. Origins can be uncertain; the map marks one representative place within the listed region.`,
...
console.log(`Wrote ${pack.cards.length} fruit cards with sizes (${source.cards.filter((card) => card.weight).length} documented whole-fruit weights).`);
